...
#             post, media = await self.sia.generate_post(
#                 platform="twitter",
#                 author=self.character.twitter_username
#             )
...
class SiaTwitterOfficial(SiaClientInterface):
        ...
        try:
            if get_message_in_db:
                log_message(
                    ...
                    "info",
                    self,
                    f"Message with id {tweet.id} already exists in the database, returning it without adding to the database",
                )
                return get_message_in_db[0]
            else:
                message_to_add = self.tweet_to_message(tweet=tweet, author=author)
            ...
            try:
                ...
                if flagged:
                    log_message(
                        ...
                        "info",
                        self,
                        f"The tweet (id {tweet.id}) ({tweet.text.replace('\n', ' ')}) was flagged: {moderation_response.results}",
                    )
            except Exception as e:
                log_message(self.logger, "error", self, f"Error moderating tweet: {e}")
                flagged = False
            ...
            if exclude_responded_to:
                ...
                if message_responses_in_db:
                    log_message(
                        ...
                        "info",
                        self,
                        f"Message with id {tweet.id} has already been responded to",
                    )
                    continue
                else:
                    if tweet_message:
    ...
    def printable_tweet(
        ...
        output_str += (
            " " * indent_width
            + f"(likes: {public_metrics.get('like_count', 0)}, retweets: {public_metrics.get('retweet_count', 0)}, replies: {public_metrics.get('reply_count', 0)}, quotes: {public_metrics.get('quote_count', 0)})"
        )
        return output_str
    ...
    def decide_which_tweet_to_reply_to(
        ...
        # Create a string version of the tweet list.
        tweets_str_for_prompt = tweets[0].printable_list(tweets)
        
        # Define a prompt that instructs the model to output JSON.
        prompt_template = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
...
Your objective is to select the most relevant tweet to respond to from the list provided below.
...
{tweets}
        ...
        # Format the prompt.
        prompt = prompt_template.format(**ai_input)

        try:
            # Run the Hugging Face generation synchronously.
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result_text = loop.run_until_complete(
                self.sia.llm.generate(prompt, max_length=300)
            )
            loop.close()
            log_message(self.logger, "info", self, f"Hugging Face raw output: {result_text}")

            # Parse the output as JSON.
            decision_dict = json.loads(result_text)
        except Exception as e:
            log_message(self.logger, "error", self, f"Error generating or parsing decision: {e}")
            return None

        try:
            # Use the parsed dictionary to extract the tweet ID.
            selected_tweet_id = decision_dict.get("tweet_id")
        except Exception as e:
            log_message(self.logger, "error", self, f"Error processing decision output: {e}")
            return None

        tweet = tweets[0].select_by_id_from_list(tweets, selected_tweet_id)
        return tweet
    ...
    async def engage(self, testing_rounds=3, search_period_hours=24):
        ...
        if messages_to_engage_in_db:
            ...
            if not is_time_to_engage and not self.testing:
                log_message(
                    ...
                    "info",
                    self,
                    f"Not the time to engage yet. Last engagement: {latest_message.wen_posted}, next engagement: {next_time_to_engage}",
                )
                return
        ...
        for i in range(testing_rounds):
            ...
            for search_query in self.character.platform_settings.get("twitter", {}).get("engage", {}).get("search_queries", []):
                ...
                if tweets is None:
                    log_message(self.logger, "error", self, f"Error fetching tweets for query: {search_query}")
                    continue
            ...
            if not tweets_to_engage:
                log_message(self.logger, "info", self, "No tweets found to engage with")
                continue
            ...
            # Select a tweet to respond to
            tweet_to_respond = self.decide_which_tweet_to_reply_to(tweets_to_engage)

            if tweet_to_respond is None:
                log_message(self.logger, "error", self, "No valid tweet found to respond to.")
                continue
            ...
            ai_response = await self.sia.generate_response(
                tweet_to_respond,
                use_filtering_rules=False,
                platform="twitter",
                previous_messages=previous_messages,
            ...
            if not self.testing:
                ...
                if not tweet_id:
                    log_message(self.logger, "error", self, "Failed to publish tweet response.")
